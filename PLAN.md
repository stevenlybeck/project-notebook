...
- **Long-polling background agent** — a `project-notebook watch <project>` command that holds a long-poll against the hub and waits for artifacts, instead of the current poll-on-prompt model (`docs/architecture.md` §4) and its deferred "push vs poll" question. Design notes worth keeping:
  - **Notify on upload *start*, not completion.** The hub already populates `active_uploads` when a PUT begins, so it can emit the event then — letting the agent assemble a context package while a large file is still uploading (latency hidden behind the transfer).
